...
/* Keep a global copy of all internal members of getopt_data_vals.  */
static getopt_data getopt_data_vals;
...
static void exchange (TCHAR **argv, getopt_data *d)
{
  ...
  /* First make sure the handling of the `__getopt_nonoption_flags'
     ...
     of the string.  */
  if ((d->__nonoption_flags_len > 0) &&
      (top >= d->__nonoption_flags_max_len))
  {
      /* We must extend the array.
         The user plays games with us and presents new arguments.  */
      TCHAR *new_str = malloc ((top + 1)*sizeof(TCHAR));

      if (new_str == NULL)
      {
          d->__nonoption_flags_len = 0;
          d->__nonoption_flags_max_len = 0;
      } else {
#ifndef __mempcpy
          memset(memcpy(new_str, __getopt_nonoption_flags,
...
static const TCHAR * _getopt_initialize (int argc, TCHAR *const *argv,
...
#if defined(_MSC_VER) && __STDC_WANT_SECURE_LIB__
  _dupenv_s(&envstr, &envstrsz, "POSIXLY_CORRECT");
  d->__posixly_correct = posixly_correct | !!envstr;
#else
...
#if defined(_LIBC) && defined(USE_NONOPTION_FLAGS)
  if ((!d->__posixly_correct) && (argc == __libc_argc)
      && (argv == __libc_argv))
  {
      if (d->__nonoption_flags_max_len == 0)
      {
          ...
          size_t len = lstrlen(orig_str);
          d->__nonoption_flags_max_len = lstrlen(orig_str);
          ...
          __getopt_nonoption_flags = (TCHAR *)malloc((d->__nonoption_flags_max_len) * sizeof(TCHAR));
...
#ifndef mempcpy
              memset(memcpy(__getopt_nonoption_flags, orig_str, len),
...
#endif /* defined(_LIBC) && defined(USE_NONOPTION_FLAGS) */
...
_getopt_internal_r (int argc, TCHAR *const *argv, const TCHAR *optstring,
                    const option_t *longopts, int *longind, int long_only,
                    getopt_data *d, int posixly_correct)
{
...
#define NONOPTION_P ((argv[d->optind][0] != _T('-')) || (argv[d->optind][1] == _T('\0')))
  ...
  if ((longopts != NULL)
                         ...
                         !_tcschr (optstring, argv[d->optind][1])))))
    {
      TCHAR *nameend = NULL;
      size_t namelen = 0;
      const option_t *p = NULL;
      const option_t *pfound = NULL;
      int exact = 0;
      int indfound = -1;
      int option_index = 0;
      ...
      if ((ambig_list != NULL) && (!exact))
      {
        if (print_errors)
	    {
          ...
          if (fp != NULL)
          {
              ...
              for (; ambig_list == NULL; ambig_list = ambig_list->next)
              {
...
#define _IO_FILE FILE * __cdecl
  ...
  /* Look at and handle the next short option-character.  */
  {
    ...
    for (p = longopts, option_index = 0; p->name; p++, option_index++)
    {
        ...
        {
            if ((int)(nameend - d->__nextchar) == lstrlen(p->name))
            {
                /* Exact match found.  */
                pfound = p;
                indfound = option_index;
...
argv[0], pfound->name);
		...
		if (d->optind < argc)
		...
		else
		  {
			...
			if (__asprintf (&buf,
                _T("%s: option '-W %s' requires an argument\n"),
                argv[0], pfound->name) >= 0)
			  {
			    _IO_flockfile (stderr);
...
int _getopt_internal (int argc, TCHAR *const *argv, const TCHAR *optstring,
                      const option_t *longopts, int *longind,
                      int long_only, int posixly_correct)
{
  int result = 0;

  getopt_data_vals.optind = optind;
  getopt_data_vals.opterr = opterr;

  result = _getopt_internal_r(argc, argv, optstring, longopts,
			                  longind, long_only, &getopt_data_vals,
			                  posixly_correct);

  optind = getopt_data_vals.optind;
  optarg = getopt_data_vals.optarg;
  optopt = getopt_data_vals.optopt;
...
{
  return _getopt_internal(argc, argv, optstring, (const option_t *)0, NULL,
                          0, 0);
}

#ifdef _LIBC
int __posix_getopt(int argc, TCHAR *const *argv, const TCHAR *optstring)
{
