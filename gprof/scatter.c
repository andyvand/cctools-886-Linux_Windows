...
void)
{
	...
	for(afile = files; afile < &files[n_files]; afile++){
        namep = strrchr(afile->name, '/');
	    if((namep = strrchr(afile->name, '/')) != NULL)
		namep++; 
	    else
		namep = afile->name;

#if defined(_MSC_VER) && __STDC_WANT_SECURE_LIB__
        strcpy_s(ar_name, 15, namep);
#else
	    ...
	    while((start = stop = strstr(whatsloadedp, ar_name)) != NULL){
...
arctype *backarc)
{ 
    UNREFERENCED_PARAMETER(arc);
    UNREFERENCED_PARAMETER(backarc);
...
void)
{
	...
	qsort(nl, npe - nl, sizeof(nltype),  
	      (int(__cdecl *)(const void *, const void *))compare_nl);
	print_nl(callf);

	/* call order order file */
	qsort(nl, npe - nl, sizeof(nltype),  
	      (int(__cdecl *)(const void *, const void *))compare_nl_order);
	print_nl(callo);

	/* time order file */
	qsort(nl, npe - nl, sizeof(nltype),  
	      (int (__cdecl *)(const void *, const void *))timecmp);
	print_nl(callt);
}
...
void)
{
	...
	if(xflag == FALSE)
#if defined(_MSC_VER) && __STDC_WANT_SECURE_LIB__
    if ((fopen_s(&gmon, "gmon.order", "w")) && (gmon == NULL))
        system_fatal("can't create file: gmon.order");
    if ((fopen_s(&callf, "callf.order", "w")) && (callf == NULL))
        system_fatal("can't create file: callf.order");
    if ((fopen_s(&callo, "callo.order", "w")) && (callo == NULL))
        system_fatal("can't create file: callo.order");
    if ((fopen_s(&callt, "callt.order", "w")) && (callt == NULL))
        system_fatal("can't create file: callt.order");
    /*
    if ((fopen_s(&treefile, "gmon.tree", "w")) && (treefile == NULL))
        system_fatal("can't create file: gmon.tree");
    */
    _sopen_s(&what_fd, "whatsloaded", O_RDONLY, SH_DENYNO, 0);
    if (what_fd >= 0) {
#else
	...
	if((treefile = fopen("gmon.tree", "w")) == NULL)
	    system_fatal("can't create file: gmon.tree");
	*/
	if((what_fd = _open("whatsloaded", O_RDONLY)) >= 0){
#endif
	...
	qsort(files, n_files, sizeof(struct file), 
	      (int(__cdecl *)(const void *, const void *))compare_file);
	scatter();
	if(xflag == FALSE)
	    fclose(gmon);
	fclose(callf);
