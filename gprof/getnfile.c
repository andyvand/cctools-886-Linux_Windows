...
static unsigned char funcsymbol(
    uint8_t n_type,
    uint8_t n_sect,
    char *name);
...
done1:		;
	...
	qsort(nl, nname, sizeof(nltype),
        (int (__cdecl *)(const void *, const void *))valcmp);
#ifdef DEBUG
	if(debug & AOUTDEBUG){
	    for(i = 0; i < nname; i++){
		printf("[getnfile] ");
...
uint64_t *text_max)
{
    ...
    struct segment_command_64 *sg64 = NULL;
    struct section *s = NULL;
    struct section_64 *s64 = NULL;
