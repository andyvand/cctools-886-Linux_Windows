 ...
 */
__private_extern__
struct fat_arch * cpusubtype_findbestarch(cpu_type_t cputype, cpu_subtype_t cpusubtype, struct fat_arch *fat_archs, uint32_t nfat_archs)
{
    uint32_t i;
	...
	case CPU_TYPE_I386:
        ...
        for (i = 0; i < nfat_archs; i++)
        {
            ...
            if ((cpu_subtype_t)(CPU_SUBTYPE_INTEL_FAMILY(fat_archs[i].cpusubtype &  ~CPU_SUBTYPE_MASK)) < (cpu_subtype_t)lowest_family)
            {
                lowest_family = CPU_SUBTYPE_INTEL_FAMILY(fat_archs[i].cpusubtype & ~CPU_SUBTYPE_MASK);
            }
        }
		...
		if(fat_archs[i].cputype != cputype)
		    continue;
		if ((cpu_subtype_t)(CPU_SUBTYPE_INTEL_FAMILY(fat_archs[i].cpusubtype &
					    ~CPU_SUBTYPE_MASK)) == (cpu_subtype_t)lowest_family){
		    if ((cpu_subtype_t)(CPU_SUBTYPE_INTEL_MODEL(fat_archs[i].cpusubtype &
					       ~CPU_SUBTYPE_MASK)) <
                (cpu_subtype_t)lowest_model)
            {
		        lowest_model = CPU_SUBTYPE_INTEL_MODEL(fat_archs[i].cpusubtype & ~CPU_SUBTYPE_MASK);
			    lowest_index = i;
		    }
		}
	    }
 ...
 */
__private_extern__
cpu_subtype_t cpusubtype_combine(cpu_type_t cputype, cpu_subtype_t cpusubtype1, cpu_subtype_t cpusubtype2)
{
	/*
