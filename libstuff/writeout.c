...
#ifndef TZCAST
#define TZCAST struct timezone *
#endif /* TZCAST */

#ifndef TVCAST
#define TVCAST struct timeval *
#endif /* TVCAST */
...
#ifndef TZCAST
#define TZCAST __timezone_ptr_t __restrict
#endif /* TZCAST */

#ifndef TVCAST
#define TVCAST struct timeval *__restrict
#endif /* TVCAST */
#endif
#endif
...
static void make_table_of_contents(
    ...
    char *output,
    time_t toc_time,
    unsigned char sort_toc,
    unsigned char commons_in_toc,
...
static unsigned char check_sort_toc_entries(
    struct arch *arch,
    char *output,
    unsigned char library_warnings);
...
uint32_t *throttle)
{
    ...
    if(throttle != NULL)
        ...
        if(output != NULL)
        {
            ...
            _sopen_s(&fd, output, _O_WRONLY | _O_CREAT | _O_TRUNC | fsyncv, _SH_DENYNO, _S_IREAD | _S_IWRITE);

            if (fd == -1) {
#elif defined(_MSC_VER)
                if ((fd = _open(output, O_WRONLY | O_CREAT | O_TRUNC | fsyncv, mode)) == -1) {
#else
            if ((fd = _open(output, O_WRONLY | O_CREAT | O_TRUNC | fsyncv, mode)) == -1) {
#endif
...
#define WRITE_SIZE (32 * 1024)
            ...
            if(gettimeofday((TVCAST)&start, (TZCAST)&tz) == -1)
                goto no_throttle;
#endif
#undef THROTTLE_DEBUG
            do {
                ...
                if(_write(fd, p, write_size) != (int)write_size){
                ...
                if((p < (file + file_size)) || (*throttle == UINT_MAX))
                {
...
#if defined(_WIN32) || defined(WIN32)
                    ftime(&tbuf);
                    time_used = (double)(tbuf2.time - tbuf.time);
#else
                    (void)gettimeofday((TVCAST)&end, (TZCAST)&tz);
                    ...
                    if(end.tv_usec >= start.tv_usec)
                        ...
                        time_used += -1.0 +
                            ...
                            1000000.0);
                    bytes_per_second = (uint32_t)((double)bytes_written / time_used);
#endif
#ifdef THROTTLE_DEBUG
                    ...
                    if(bytes_per_second > *throttle){
                        ...
                        ftime(&tbuf2);
#else
                        (void)gettimeofday((TVCAST)&start, (TZCAST)&tz);
#endif
                    }
                }
            ...
            no_throttle:
#endif
	    if(_write(fd, file, file_size) != (int)file_size){
		system_error("can't write output file: %s", output);
		goto cleanup;
	    }
	}
	if((output != NULL) && (_close(fd) == -1))
    {
...
unsigned char *seen_archive)
{
	...
	*seen_archive = FALSE;
	...
	for(i = 0; i < narchs; i++){
		...
		*seen_archive = TRUE;
		...
		if(archs[i].fat_arch != NULL)
		    file_size = (uint32_t)rnd(file_size, (uint64_t)(1 << archs[i].fat_arch->align));
	...
	if(narchs > 1 || archs[0].fat_arch != NULL){
        ...
        offset = (uint32_t)rnd(offset, (uint64_t)(1 << archs[i].fat_arch->align));
	...
	for(i = 0; i < narchs; i++){
		...
		*seen_archive = TRUE;
		...
		for(j = 0; j < archs[i].nmembers; j++){
			...
			pad = (uint32_t)rnd(archs[i].members[j].unknown_size, 8) -
				    archs[i].members[j].unknown_size;
		    }
...
unsigned char library_warnings)
{
	...
	arch->toc_strsize = (uint32_t)rnd(arch->toc_strsize, 8);
	...
	for(i = 0; i < arch->nmembers; i++){
		...
		for(j = 0; j < nsymbols; j++){
			...
			if(toc_symbol(symbols + j, commons_in_toc,
			   object->sections) == TRUE){
#if defined(_MSC_VER) && __STDC_WANT_SECURE_LIB__
                strcpy_s(arch->toc_strings + s, arch->toc_strsize - s, strings + symbols[j].n_un.n_strx);
#else
			    strncpy(arch->toc_strings + s, 
				   strings + symbols[j].n_un.n_strx, arch->toc_strsize - s);
#endif
			...
			if(toc_symbol_64(symbols64 + j, commons_in_toc,
			   object->sections64) == TRUE){
#if defined(_MSC_VER) && __STDC_WANT_SECURE_LIB__
                strcpy_s(arch->toc_strings + s, arch->toc_strsize - s,
		...
		if(object->output_sym_info_size == 0){
			...
			if(object->mh != NULL)
                ...
                for(j = 0; j < nsymbols; j++){
                    if(lto_toc_symbol(member->lto, j, commons_in_toc) == TRUE){
#if defined(_MSC_VER) && __STDC_WANT_SECURE_LIB__
            strcpy_s(arch->toc_strings + s, arch->toc_size - s,             lto_symbol_name(member->lto, j));
#else
			strncpy(arch->toc_strings + s,
			       lto_symbol_name(member->lto, j), arch->toc_size - s);
#endif
	...
	if(sort_toc == TRUE){
	    qsort(arch->toc_entries, arch->ntocs, sizeof(struct toc_entry),
		  (int (__cdecl *)(const void *, const void *))toc_entry_name_qsort);
	...
	if(arch->toc_long_name == TRUE)
	    arch->toc_size += (uint32_t)(arch->toc_name_size +
			      (rnd(sizeof(struct ar_hdr), 8) -
			       sizeof(struct ar_hdr)));
	for(i = 0; i < arch->nmembers; i++)
...
#if defined(_MSC_VER) && __STDC_WANT_SECURE_LIB__
    _umask_s(numask, &oumask);
    toc_mode = S_IFREG | (0666 & ~oumask);
    _umask_s(oumask, NULL);

    sprintf_s((char *)(&arch->toc_ar_hdr), arch->toc_size, "%-*s%-*ld%-*u%-*u%-*o%-*ld",
#else
    oumask = _umask(numask);
    toc_mode = S_IFREG | (0666 & ~oumask);
    (void)_umask(oumask);

    snprintf((char *)(&arch->toc_ar_hdr), arch->toc_size, "%-*s%-*ld%-*u%-*u%-*o%-*ld",
#endif
