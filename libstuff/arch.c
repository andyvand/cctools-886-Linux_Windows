...
#include "../include/stuff/openstep_mach.h"
#include "../include/stuff/allocate.h"
...
int get_arch_from_flag(char *name, struct arch_flag *arch_flag)
{
    ...
    {
	    if(strncmp(arch_flags[i].name, name, strlen(arch_flags[i].name) + 1) == 0)
        {
            if (arch_flag != NULL)
            {
 ...
 */
__private_extern__
const char * get_arch_name_from_types(cpu_type_t cputype, cpu_subtype_t cpusubtype)
{
    uint32_t i = 0;
    char *p = NULL;
    ...
    if (p != NULL)
    {
        ...
        sprintf_s(p, strlen(p), "cputype %u cpusubtype %u", cputype,
                  cpusubtype & ~CPU_SUBTYPE_MASK);
#else
        snprintf(p, strlen(p), "cputype %u cpusubtype %u", cputype,
                 cpusubtype & ~CPU_SUBTYPE_MASK);
#endif
    }
#else
 ...
 */
__private_extern__
const struct arch_flag * get_arch_family_from_cputype(cpu_type_t cputype)
{
    uint32_t i = 0;
 ...
 */
__private_extern__
enum byte_sex get_byte_sex_from_flag(const struct arch_flag *flag)
{
 ...
 */
__private_extern__
int get_stack_direction_from_flag(const struct arch_flag *flag)
{
    if ((flag->cputype == CPU_TYPE_MC680x0) ||
        (flag->cputype == CPU_TYPE_MC88000) ||
        (flag->cputype == CPU_TYPE_POWERPC) ||
 ...
 */
__private_extern__
uint64_t get_stack_addr_from_flag(const struct arch_flag *flag)
{
    switch(flag->cputype)
    {
 ...
 */
__private_extern__
uint32_t get_stack_size_from_flag(const struct arch_flag *flag)
{
 ...
 */
__private_extern__
uint32_t get_segalign_from_flag(const struct arch_flag *flag)
{
 ...
 */
__private_extern__
vm_prot_t get_segprot_from_flag(const struct arch_flag *flag)
{
 ...
 */
__private_extern__
uint32_t get_shared_region_size_from_flag(const struct arch_flag *flag)
{
	if(flag->cputype == CPU_TYPE_ARM)
	   return (0x08000000);
...
__private_extern__
#if defined(_WIN32) || defined(WIN32)
bool force_cpusubtype_ALL_for_cputype(cpu_type_t cputype)
#else
bool force_cpusubtype_ALL_for_cputype(cpu_type_t cputype)
#endif
{
