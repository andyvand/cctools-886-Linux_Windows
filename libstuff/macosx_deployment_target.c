...
#if defined(_MSC_VER) && __STDC_WANT_SECURE_LIB__
        _dupenv_s(&p, &plen, "MACOSX_DEPLOYMENT_TARGET");
#else
...
#if defined(_MSC_VER) && __STDC_WANT_SECURE_LIB__
        strcpy_s(value->name, strlen(p), p);
#else
	    strncpy(value->name, p, strlen(p));
#endif
...
#if defined(_WIN32) || defined(WIN32)
    sprintf_s(value->name, 32, "10.%u.%u", major, minor);
#else
	snprintf(value->name, 32, "10.%u.%u", major, minor);
#endif
...
#if defined(_MSC_VER) && __STDC_WANT_SECURE_LIB__
    strcpy_s(value->name, 4, "10.6");
#else
	strncpy(value->name, "10.6", 4);
#endif
