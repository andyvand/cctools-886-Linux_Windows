...
#if defined(_WIN32) || defined(WIN32)
static HMODULE lto_handle = NULL;
#else
   ...
   void *mod;
	...
	if(tried_to_load_lto == 0){
...
#if defined(_WIN32) || defined(WIN32)
        lto_path = "LTO.dll";
        lto_handle = GetModuleHandleA(lto_path);
#else
	    bufsize = MAXPATHLEN;
	    p = buf;
        ...
        if (lto_handle == NULL) {
            ...
            lto_path = NULL;
#if defined(__linux__)
            lto_handle = dlopen("/usr/lib/llvm-3.8/lib/libLTO.so", RTLD_NOW);
	    ...
	    lto_is_object = (int(__stdcall *)(const void *, size_t))dlsym(lto_handle,
				  "lto_module_is_object_file_in_memory");
	    lto_create = (lto_module_t(__stdcall *)(const void *, size_t))dlsym(lto_handle, "lto_module_create_from_memory");
	    lto_create_local = (lto_module_t(__stdcall *)(const void *, size_t, const char *))dlsym(lto_handle,
				     "lto_module_create_in_local_context");
	    lto_dispose = (void(__stdcall *)(void *))dlsym(lto_handle, "lto_module_dispose");
	    lto_get_target = (char *(__stdcall *)(void *))dlsym(lto_handle, "lto_module_get_target_triple");
	    lto_get_num_symbols = (uint32_t(__stdcall *)(void *))dlsym(lto_handle,
					"lto_module_get_num_symbols");
	    lto_get_sym_attr = (lto_symbol_attributes(__stdcall *)(void *, uint32_t))dlsym(lto_handle,
				     "lto_module_get_symbol_attribute");
	    lto_get_sym_name = (char *(__stdcall *)(void *, uint32_t))dlsym(lto_handle, "lto_module_get_symbol_name");
...
char *target_triple)
{
    ...
    while ((target_triple[i] != '-') && (i != MAX_PATH))
    {
        p[i] = target_triple[i];
    }
#else
