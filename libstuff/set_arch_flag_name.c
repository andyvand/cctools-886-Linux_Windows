...
struct arch_flag *p)
{
	...
	if(p->name == NULL){
        ...
        sprintf_s(p->name, strlen(savestr("cputype (1234567890) cpusubtype (1234567890)")), "cputype (%d) cpusubtype (%d)",
                 p->cputype, p->cpusubtype);
#else
	    snprintf(p->name, strlen(savestr("cputype (1234567890) cpusubtype (1234567890)")), "cputype (%d) cpusubtype (%d)",
		    p->cputype, p->cpusubtype);
#endif
	}
}
