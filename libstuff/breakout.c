...
struct ofile *ofile)
{
	...
	if(ofile->file_type == OFILE_FAT && errors == 0){
	    ...
	    arch->lto = ofile->lto;
	    arch->unknown_addr = ofile->file_addr;
	    arch->unknown_size = (uint32_t)ofile->file_size;
	}
#endif /* LTO_SUPPORT */
...
struct ofile *ofile)
{
	...
	if((flag = ofile_first_member(ofile)) == TRUE && errors == 0){
		...
		*ar_hdr = *(ofile->member_ar_hdr);
        ...
        sprintf_s(ar_name_buf, sizeof(ar_name_buf), "%s%-*lu", AR_EFMT1,
#else
        snprintf(ar_name_buf, sizeof(ar_name_buf), "%s%-*lu", AR_EFMT1,
#endif
...
#if defined(_MSC_VER) && __STDC_WANT_SECURE_LIB__
        sprintf_s(ar_size_buf, sizeof(ar_size_buf), "%-*ld",
#else
        snprintf(ar_size_buf, sizeof(ar_size_buf), "%-*ld",
#endif
