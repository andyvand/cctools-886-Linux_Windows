...
__private_extern__ unsigned char multi_module_dylib = TRUE;
#ifndef RLD
static unsigned char filetype_specified = FALSE;
static unsigned char moduletype_specified = FALSE;
/* if the -A flag is specified use to set the object file type */
static unsigned char Aflag_specified = FALSE;
#endif /* !defined(RLD) */
/*
...
#elif __x86_64__
    { "x86_64", CPU_TYPE_X86_64,    CPU_SUBTYPE_I386_ALL };
#elif __arm__
...
#error "unsupported architecture for static KLD"
    ...
    { 0 };
#endif /* defined(KLD) && defined(__STATIC__) */
__private_extern__ unsigned char specific_arch_flag = FALSE;

/*
 * The -force_cpusubtype_ALL flag.
 */
__private_extern__ unsigned char force_cpusubtype_ALL = FALSE;

/* the byte sex of the output file */
__private_extern__ enum byte_sex target_byte_sex = UNKNOWN_BYTE_SEX;
static unsigned char arch_multiple = FALSE;	/* print one arch message before error*/

__private_extern__
unsigned char trace = FALSE;		/* print stages of link-editing */
__private_extern__
unsigned char save_reloc = FALSE;		/* save relocation information */
__private_extern__
unsigned char output_for_dyld = FALSE;	/* produce output for use with dyld */
__private_extern__
unsigned char bind_at_load = FALSE;		/* mark the output for dyld to be bound
					   when loaded */
__private_extern__
unsigned char no_fix_prebinding = FALSE;	/* mark the output for dyld to never
					   run fix_prebinding */
__private_extern__
unsigned char load_map = FALSE;		/* print a load map */
__private_extern__
unsigned char define_comldsyms = TRUE;	/* define common and link-editor defined
...
#endif /* !defined(RLD) */
__private_extern__
unsigned char seglinkedit = FALSE;		/* create the link edit segment */
#ifndef RLD
static unsigned char
...
#endif /* !defined(RLD) */
__private_extern__
unsigned char whyload = FALSE;		/* print why archive members are
					   loaded */
#ifndef RLD
static unsigned char whatsloaded = FALSE;	/* print which object files are loaded*/
#endif /* !defined(RLD) */
__private_extern__
unsigned char flush = TRUE;			/* Use the output_flush routine to flush
					   output file by pages */
__private_extern__
unsigned char sectorder_detail = FALSE;	/* print sectorder warnings in detail */
__private_extern__
unsigned char nowarnings = FALSE;		/* suppress warnings */
__private_extern__
unsigned char no_arch_warnings = FALSE;	/* suppress wrong arch warnings */
__private_extern__
unsigned char arch_errors_fatal = FALSE;	/* cause wrong arch errors to be fatal*/
__private_extern__
unsigned char archive_ObjC = FALSE;		/* objective-C archive semantics */
__private_extern__
unsigned char archive_all = FALSE;		/* always load everything in archives */
__private_extern__
unsigned char keep_private_externs = FALSE;	/* don't turn private externs into
					   non-external symbols */
/* TRUE if -dynamic is specified, FALSE if -static is specified */
__private_extern__
unsigned char dynamic = TRUE;
#ifndef RLD
static unsigned char dynamic_specified = FALSE;
static unsigned char static_specified = FALSE;
#endif
...
__private_extern__ unsigned char strip_base_symbols = FALSE;

/* strip dead blocks */
__private_extern__ unsigned char dead_strip = FALSE;
/* don't strip module init and term sections */
__private_extern__ unsigned char no_dead_strip_inits_and_terms = FALSE;
/* print timings for dead striping code */
__private_extern__ unsigned char dead_strip_times = FALSE;
...
#ifndef RLD
static unsigned char undefined_flag_specified = FALSE;
#endif
...
__private_extern__ unsigned char nomultidefs = FALSE;
#ifndef RLD
static unsigned char multiply_defined_flag_specified = FALSE;
static unsigned char multiply_defined_unused_flag_specified = FALSE;
#endif
...
#ifndef RLD
static unsigned char prebinding_flag_specified = FALSE;
#endif
__private_extern__ unsigned char prebinding = FALSE;
__private_extern__ unsigned char prebind_allow_overlap = FALSE;
__private_extern__ unsigned char prebind_all_twolevel_modules = FALSE;
#ifndef RLD
static unsigned char read_only_reloc_flag_specified = FALSE;
static unsigned char sect_diff_reloc_flag_specified = FALSE;
static unsigned char weak_reference_mismatches_specified = FALSE;
static unsigned char prebind_all_twolevel_modules_specified = FALSE;
static unsigned char unprebound_library(
    char *dylib_install_name,
    char *seg_addr_table_filename);
#endif

/* True if -m is specified to allow multiply symbols, as a warning */
__private_extern__ unsigned char allow_multiply_defined_symbols = FALSE;
...
#ifndef RLD
__private_extern__ unsigned char segalign_specified = FALSE;
#endif /* !defined(RLD) */
...
__private_extern__ unsigned char seg1addr_specified = FALSE;

/* read-only and read-write segment addresses */
__private_extern__ unsigned long segs_read_only_addr = 0;
__private_extern__ unsigned char segs_read_only_addr_specified = FALSE;
__private_extern__ unsigned long segs_read_write_addr = 0;
__private_extern__ unsigned char segs_read_write_addr_specified = FALSE;
...
__private_extern__ unsigned char stack_addr_specified = FALSE;
__private_extern__ unsigned long stack_size = 0;
__private_extern__ unsigned char stack_size_specified = FALSE;

/* TRUE if -allow_stack_execute is specified */
__private_extern__ unsigned char allow_stack_execute = FALSE;

#ifndef RLD
/* A -segaddr option was specified */
static unsigned char segaddr_specified = FALSE;
#endif /* !defined(RLD) */
...
#ifndef RLD
static unsigned char headerpad_specified = FALSE;
#endif /* !defined(RLD) */
/*
 ...
 */
__private_extern__ unsigned char headerpad_max_install_names = FALSE;
...
__private_extern__ unsigned char sub_framework = FALSE;
__private_extern__ unsigned char umbrella_framework = FALSE;
...
__private_extern__ unsigned char namespace_specified = FALSE;
__private_extern__ unsigned char twolevel_namespace = TRUE;
__private_extern__ unsigned char force_flat_namespace = FALSE;

#ifndef RLD
/* Variable to support options logging.  */
static unsigned char ld_print_options = FALSE;
#endif
 ...
 */
#ifndef RLD
static unsigned char twolevel_namespace_hints_specified = FALSE;
#endif
__private_extern__ unsigned char twolevel_namespace_hints = TRUE;
...
static unsigned char ispoweroftwo(unsigned long x);
static vm_prot_t getprot(char *prot, char **endp);
static unsigned char check_max_init_prot(vm_prot_t maxprot, vm_prot_t initprot);
...
char *envp[])
{
    ...
    unsigned char is_framework;
    char *has_suffix;
    struct symbol_list *sp;
    char *exported_symbols_list, *unexported_symbols_list;
    unsigned char missing_syms;
    unsigned char vflag;
    unsigned char prebinding_via_LD_PREBIND;
    unsigned char hash_instrument_specified;
	...
	for(i = 1 ; i < argc ; i++){
		...
		case 's':
			...
			if(stat_buf.st_size != 0){
                            ...
                            sect_spec->file_addr = malloc(stat_buf.st_size);
                            if (sect_spec->file_addr == NULL)
				mach_fatal(r, "can't malloc file address");

			    if (stat_buf.st_size != _read(fd, sect_spec->file_addr, stat_buf.st_size))
			...
			if(stat_buf.st_size != 0){
#if defined(_WIN32) || defined(WIN32) || defined(__linux__)
                            sect_spec->order_addr = malloc(stat_buf.st_size);
                            if (sect_spec->order_addr == NULL)
				mach_fatal(r, "can't malloc file address");

			    if (stat_buf.st_size != _read(fd, sect_spec->order_addr, stat_buf.st_size))
				mach_fatal(r, "can't map file: %s for %s %s %s",
				    argv[i+3], argv[i], argv[i+1], argv[i+2]);
#else
			    if((r = map_fd((int)fd, (vm_offset_t)0,
...
unknown_flag:
	...
	for(i = 1 ; i < argc ; i++){
		...
		case 'f':
			...
			if(stat_buf.st_size != 0){
#if defined(_WIN32) || defined(WIN32) || defined(__linux__)
                            addr = malloc(stat_buf.st_size);
                            if (addr == NULL)
				mach_fatal(r, "can't malloc file address");

			    if (stat_buf.st_size != _read(fd, addr, stat_buf.st_size))
				mach_fatal(r, "can't map file: %s for %s %s %s",
				    argv[i+3], argv[i], argv[i+1], argv[i+2]);
#else
			    if((r = map_fd((int)fd, (vm_offset_t)0,
...
char *seg_addr_table_filename)
{
	...
	if(stat_buf.st_size != 0){
#if defined(_WIN32) || defined(WIN32) || defined(__linux__)
	    file_addr = malloc(stat_buf.st_size);

	    if (file_addr == NULL)
		mach_fatal(r, "can't malloc %lu bytes for LD_UNPREBOUND_LIBRARIES environment variable", (unsigned long)stat_buf.st_size);

	    if (stat_buf.st_size != _read(fd, file_addr, stat_buf.st_size))
...
{
    static unsigned char printed = FALSE;
