...
#ifndef RLD
static unsigned char check_reserved_segment(char *segname,
...
layout_rld_symfile(void)
{
	...
	output_addr = malloc(output_size);
	if (output_addr == NULL)
	    mach_fatal(r, "can't malloc() memory for output of size "
		       "%lu", output_size);
#else
...
layout_segments(void)
{
    ...
    unsigned char address_zero_specified;
...
#else /* !defined(SA_RLD) */m
#if defined(_WIN32) || defined(WIN32) || defined(__linux__)
	output_addr = malloc(allocate_size);
	if (output_addr == NULL)
	    mach_fatal(r, "can't malloc() memory for output of size "
			"%lu", allocate_size);
#else
