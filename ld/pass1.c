...
__private_extern__ unsigned char search_paths_first = FALSE;
 ...
 */
static unsigned char indirect_dylib = FALSE;
...
static void pass1_fat(
    ...
    unsigned char dylib_only,
    unsigned char bundle_loader,
    unsigned char force_weak);
...
static void pass1_archive(
    ...
    unsigned long file_size,
    unsigned char base_name,
    unsigned char from_fat_file,
    unsigned char bundle_loader,
    unsigned char force_weak);

static unsigned char check_archive_arch(
    char *file_name,
    char *file_addr,
    unsigned long file_size);

static unsigned char check_extend_format_1(
    char *name,
...
static void check_dylibs_for_definition(
    ...
    unsigned char prebind_check,
    unsigned char twolevel_namespace_check);
static unsigned char check_dylibs_for_reference(
    struct merged_symbol *merged_symbol);

static unsigned char open_dylib(
    struct dynamic_library *p);

static unsigned char set_sub_frameworks_ordinals(
    struct dynamic_library *umbrella);

static unsigned char set_sub_umbrella_sub_library_ordinal(
...
unsigned char force_weak)
{
	...
	file_addr = malloc(file_size);

	if (file_addr == NULL){
            close(fd);
	    mach_fatal(r, "can't malloc file address: %s", file_name);
	}

	if (file_size != _read(fd, file_addr, file_size)){
	    close(fd);
	...
	if(fat_header->magic == __swab32(FAT_MAGIC))
#else
...
unsigned char force_weak)
{
    ...
    struct merged_symbol *merged_symbol;
    unsigned char member_loaded;
    enum byte_sex toc_byte_sex;
    unsigned char ld_trace_archive_printed;
...
void)
{
    ...
    unsigned char removed, some_images_setup;
    char *ar_name;
    ...
    unsigned char bind_at_load_warning;
    ...
    unsigned char is_framework, set_some_ordinals, non_set_ordinals;
...
unsigned char twolevel_namespace_check)
{
    struct dynamic_library *p;
    struct dylib_table_of_contents *toc;
    static unsigned char printed_override, printed_unused, merged_symbol_printed;
...
struct dynamic_library *umbrella)
{
    unsigned char set_some_ordinals;
    struct dynamic_library *p;
    unsigned long i;
    struct mach_header *mh;
    struct load_command *lc;
...
unsigned char bundle_loader)
{
...
#ifndef RLD
    unsigned char is_framework, allowable_client;
	 ...
	 */
	cur_obj->dylib = (unsigned char)(mh->filetype == MH_DYLIB ||
				     mh->filetype == MH_DYLIB_STUB);
	dlid = NULL;
	dylib_id_name = NULL;
	 ...
	 */
	cur_obj->dylinker = (unsigned char)(mh->filetype == MH_DYLINKER);
	dyldid = NULL;
