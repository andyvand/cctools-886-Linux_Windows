...
struct object_file {
    ...
    unsigned char swapped;		/* TRUE if the bytesex of the object does not */
				/*  match the host bytesex. */
    unsigned char fvmlib_stuff;	/* TRUE if any SG_FVMLIB segments or any */
				/*  LC_LOADFVMLIB or LC_IDFVMLIB load */
				/*  commands in the file. */
    unsigned char dylib;		/* TRUE if mh->filetype == MH_DYLIB */
    unsigned char dylib_stuff;	/* TRUE if any LC_LOAD_DYLIB or LC_ID_DYLIB */
				/*  load commands in the file. */
    unsigned char bundle_loader;	/* TRUE this the -bundle_loader object */
    ...
    unsigned char dylinker;		/* TRUE if mh->filetype == MH_DYLINKER */
    unsigned char command_line;	/* TRUE if object is created for a symbol */
	...
	*cur_section_map;	/*  that is being processed from this object */
    ...
    unsigned char user_obj_addr;	/* TRUE if the user allocated the object */
				/*  file's memory (then rld doesn't */
				/*  deallocate it) */
    unsigned char from_fat_file;	/* TRUE if the object is in a fat file (then */
	...
	*objc_module_info;	/*  __module_info) section if any. */
    unsigned char init_module_loaded; /* has the module with the library */
...
struct section_map {
	...
	*fine_relocs;		/*  items which are smaller than the section. */
    ...
    unsigned char no_load_order;	/* Not to be scattered, loaded normalily */
	...
	*load_orders;
    ...
    unsigned char start_section;	/* There is a symbol at the start of the */
				/*  section. */
    unsigned char			/* For symbol stub sections if any indirect */
	absolute_indirect_defineds; /* symbol is defined as an absolute */
    unsigned char			/* TRUE when the input relocation entries */
...
struct load_order {
    ...
    unsigned char global_coalesced_symbol;
};
...
__private_extern__ void print_whatsloaded(
    void);
__private_extern__ unsigned char is_dylib_module_loaded(
...
__private_extern__ void fine_reloc_output_ref(
    ...
    unsigned long input_offset,
    struct live_ref *ref);
__private_extern__ unsigned char fine_reloc_offset_in_output(
    struct section_map *map,
    unsigned long input_offset);
__private_extern__ unsigned char fine_reloc_offset_in_output_for_output_offset(
    struct section_map *map,
    unsigned long output_offset);
__private_extern__ unsigned long fine_reloc_output_sectnum(
    struct section_map *map,
    unsigned long input_offset);
__private_extern__ unsigned char fine_reloc_arm(
    struct section_map *map,
    unsigned long input_offset);
__private_extern__ unsigned char fine_reloc_thumb(
    struct section_map *map,
    unsigned long input_offset);
__private_extern__ unsigned char fine_reloc_local(
    struct section_map *map,
    unsigned long input_offset);
