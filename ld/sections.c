 ...
 * appear in the headers of the input files.  It builds a merged section tableen
 ...
 */
__private_extern__ unsigned char some_non_subsection_via_symbols_objects = FALSE;
...
static void reloc_output_for_dyld(
    ...
    unsigned long *nlocrel,
    unsigned long *nextrel);
static unsigned char is_merged_section_read_only(
...
static void walk_references_in_section(
    ...
    struct merged_section *ms);
static unsigned char walk_references(
    enum walk_references_operation operation,
    struct fine_reloc *fine_reloc,
    struct section_map *map,
    struct object_file *obj);
static unsigned char ref_operation(
    enum walk_references_operation operation,
    struct ref *ref,
    struct object_file *obj);
...
{
    unsigned char ordered_sections;
    struct merged_segment **p, *msg;
...
struct merged_section *ms)
{
    ...
    struct load_order *load_orders;
    unsigned char start_section, any_order;
...
unsigned long *nextrel)
{
    ...
    unsigned long r_address, r_extern, r_type, r_scattered, r_pcrel,
		  r_symbolnum, r_value;
    unsigned char partial_section, sectdiff, pic, has_sect_diff_relocs;
    unsigned char flag_relocs, first_time;
