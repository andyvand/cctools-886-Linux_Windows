...
#ifndef symbol_fatal
#define symbol_fatal(X,...) { fprintf(stderr,X); exit(-1) }
#endif

#ifndef symbol_errors
#define symbol_errors(X,...) { fprintf(stderr,X); exit(-1) }
#endif
#else
#include <sys/uio.h>
#endif
...
static unsigned char ispoweroftwo(uint32_t x);
...
int __cdecl main(int argc, char **argv/*, char **envp*/)
{
    ...
    uint32_t narchs;
    char *endp;
    unsigned char section_alignment_specified, align_specified;
...
/* i386 thread states on older releases */
	...
	for(i = 0; i < arch->object->mh->ncmds; i++){
		...
		if(strcmp(sg->segname, SEG_TEXT) == 0){
            ...
            strcpy_s(scnhdrs[j].s_name, sizeof(scnhdrs[j].s_name), ".text");
#else
            strncpy(scnhdrs[j].s_name, ".text", sizeof(scnhdrs[j].s_name));
#endif
		    ...
		    scnhdrs[j].s_size = (uint32_t)rnd(sg->filesize, file_alignment);
...
#if defined(_MSC_VER) && __STDC_WANT_SECURE_LIB__
            strcpy_s(scnhdrs[j].s_name, sizeof(scnhdrs[j].s_name), ".data");
#else
		    strncpy(scnhdrs[j].s_name, ".data", sizeof(scnhdrs[j].s_name));
#endif
...
#ifndef HACK_TO_MATCH_TEST_CASE
#if defined(_MSC_VER) && __STDC_WANT_SECURE_LIB__
		    strcpy_s(scnhdrs[j].s_name, sizeof(scnhdrs[j].s_name), ".import");
#else
            strncpy(scnhdrs[j].s_name, ".import", sizeof(scnhdrs[j].s_name));
#endif
		    scnhdrs[j].s_vsize = sg->vmsize;
		    scnhdrs[j].s_vaddr = sg->vmaddr;
		    scnhdrs[j].s_size = (uint32_t)rnd(sg->filesize, file_alignment);
	...
	if(reloc_size != 0){
#if defined(_MSC_VER) && __STDC_WANT_SECURE_LIB__
        strcpy_s(scnhdrs[j].s_name, sizeof(scnhdrs[j].s_name), ".reloc");
#else
        strncpy(scnhdrs[j].s_name, ".reloc", sizeof(scnhdrs[j].s_name));
#endif
	    scnhdrs[j].s_vsize = reloc_size;
	    reloc_addr = (uint32_t)rnd(reloc_addr, section_alignment);
	    scnhdrs[j].s_vaddr = reloc_addr;
	    scnhdrs[j].s_size = (uint32_t)rnd(reloc_size, file_alignment);
	    scnhdrs[j].s_relptr = 0;
	    scnhdrs[j].s_lnnoptr = 0;
	    scnhdrs[j].s_nlnno = 0;
	    scnhdrs[j].s_flags = IMAGE_SCN_MEM_READ |
	    ...
	    debug_addr = (uint32_t)rnd(reloc_addr, section_alignment);
	}

	if(debug_filename != NULL){
#if defined(_MSC_VER) && __STDC_WANT_SECURE_LIB__
        strcpy_s(scnhdrs[j].s_name, sizeof(scnhdrs[j].s_name), ".debug");
#else
        strncpy(scnhdrs[j].s_name, ".debug", sizeof(scnhdrs[j].s_name));
#endif
	    scnhdrs[j].s_vsize = debug_size;
	    scnhdrs[j].s_vaddr = debug_addr;
	    scnhdrs[j].s_size = (uint32_t)rnd(debug_size, file_alignment);
	    scnhdrs[j].s_relptr = 0;
	    scnhdrs[j].s_lnnoptr = 0;
	    scnhdrs[j].s_nlnno = 0;
	    scnhdrs[j].s_flags = IMAGE_SCN_MEM_READ |
...
struct arch *arch)
{
	 ...
	 * copied to the start of the string table.
	 */
	section_names = ((char *)allocate(section_names_size) + 1);
	section_name = section_names;
	if(section_names_size != 0)
	    *section_name = '\0';
	...
	for(i = 0; i < arch->object->mh64->ncmds; i++){
		...
		if(strcmp(sg64->segname, SEG_TEXT) == 0){
		    ...
		    scnhdrs[j].s_size = (uint32_t)rnd(sg64->filesize, file_alignment);
		    scnhdrs[j].s_relptr = 0;
		    scnhdrs[j].s_lnnoptr = 0;
		    scnhdrs[j].s_nlnno = 0;
		    scnhdrs[j].s_flags = IMAGE_SCN_MEM_EXECUTE |
		...
		for(i = 0; i < sg64->nsects; i++, s64++){
#if defined(_MSC_VER) && __STDC_WANT_SECURE_LIB__
            sprintf_s(scnhdrs[j].s_name, sizeof(scnhdrs[j].s_name), "/%d", strsize);
            strcat_s(section_name, section_names_size - strlen(section_name), "LC_SEGMENT.");
            strcat_s(section_name, section_names_size - strlen(section_name), s64->segname);
            strcat_s(section_name, section_names_size - strlen(section_name), ".");
            strcat_s(section_name, section_names_size - strlen(section_name), s64->sectname);
#else
		    snprintf(scnhdrs[j].s_name, sizeof(scnhdrs[j].s_name), "/%d", strsize);
            strncat(section_name, "LC_SEGMENT.", section_names_size - strlen(section_name));
            strncat(section_name, s64->segname, section_names_size - strlen(section_name));
            strncat(section_name, ".", section_names_size - strlen(section_name));
            strncat(section_name, s64->sectname, section_names_size - strlen(section_name));
#endif
		    ...
		    *section_name = '\0'; /* start of next section name */
		    ...
		    scnhdrs[j].s_size = (uin32_t)rnd(s64->size, file_alignment);
		    scnhdrs[j].s_relptr = 0;
		    scnhdrs[j].s_lnnoptr = 0;
		    scnhdrs[j].s_nlnno = 0;
		    scnhdrs[j].s_flags = IMAGE_SCN_MEM_EXECUTE |
...
struct ofile *ofile)
{
	...
	header_size = (uint32_t)rnd(header_size, file_alignment);
#ifdef HACK_TO_MATCH_TEST_CASE
	...
	filehdr.f_nscns = (uint16_t)nscns;
	...
	if(ofile->mh != NULL){
       ...
       */
	    ...
	    aouthdr.dsize = 0;
	    aouthdr.bsize = 0;
	    aouthdr.SizeOfImage = (uint32_t)rnd(header_size, section_alignment);
	    for(i = 0; i < nscns; i++){
	        aouthdr.SizeOfImage += (uint32_t)rnd(scnhdrs[i].s_vsize, section_alignment);
		}
       ...
       */
	    ...
	    aouthdr64.SizeOfImage = (uint32_t)rnd(header_size, section_alignment);
	    for(i = 0; i < nscns; i++){
	        aouthdr64.SizeOfImage += (uint32_t)rnd(scnhdrs[i].s_vsize, section_alignment);
	    }
#ifdef HACK_TO_MATCH_TEST_CASE
...
char *out)
{
	 ...
	 */
#if defined(_MSC_VER) && __STDC_WANT_SECURE_LIB__
    _sopen_s(&f, out, O_WRONLY | O_CREAT | O_TRUNC, SH_DENYNO, 0644);
#else
	f = _open(out, O_WRONLY|O_CREAT|O_TRUNC, 0644);
#endif

	if(f == -1)
	    system_fatal("Can't create output file: %s", out);

	if(_write(f, buf, output_size) != (int)output_size)
...
struct arch *arch)
{
	...
	if(dyst != NULL && dyst->nlocrel != 0){
	    ...
	    if(arch->object->mh_cputype == CPU_TYPE_I386)
		gather_base_reloc_info((uint32_t)first_addr, relocs, dyst->nlocrel,
		    CPU_TYPE_I386, 2, GENERIC_RELOC_VANILLA,
		    IMAGE_REL_BASED_HIGHLOW);
	    else if(arch->object->mh_cputype == CPU_TYPE_POWERPC)
...
int base_reloc_type)
{
	...
	for(i = 0; i < nreloc; i++){
	    ...
	    if((r_extern == 0) && (r_pcrel == 0) &&
	       (r_length == (uint32_t)length) && (r_type == (uint32_t)macho_reloc_type))
...
void)
{
	...
	qsort(base_relocs, nbase_reloc, sizeof(struct base_reloc),
	      (int (__cdecl *)(const void *, const void *))cmp_base_relocs);
	...
	for(i = 0; i < (int)nbase_reloc; i++){
	    ...
	    b[entries].type = (uint16_t)base_relocs[i].type;
	    b[entries].offset = (uint16_t)offset;
	    entries++;
	}
...
#if defined(_MSC_VER) && __STDC_WANT_SECURE_LIB__
    strcpy_s(p, debug_size, debug_filename);
#else
	strncpy(p, debug_filename, debug_size);
#endif
}
...
#if defined(_MSC_VER) && __STDC_WANT_SECURE_LIB__
    count = (uint8_t)sscanf_s(string, UUID_FORMAT_STRING,
             &uuid[0], &uuid[1], &uuid[2], &uuid[3],
             &uuid[4], &uuid[5], &uuid[6], &uuid[7],
             &uuid[8], &uuid[9], &uuid[10], &uuid[11],
             &uuid[12], &uuid[13], &uuid[14], &uuid[15]);
#else
	count = (uint8_t)sscanf (string, UUID_FORMAT_STRING,
	&uuid[0], &uuid[1], &uuid[2], &uuid[3],
	&uuid[4], &uuid[5], &uuid[6], &uuid[7],
	&uuid[8], &uuid[9], &uuid[10], &uuid[11],
	&uuid[12], &uuid[13], &uuid[14], &uuid[15]);
