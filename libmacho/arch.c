...
cpu_subtype_t cpusubtype)
{
	...
	q->description = malloc(sizeof("Intel family  model ") + 10);
	if(q->description == NULL){
	    free(q);
	    return(NULL);
	}
#if defined(_MSC_VER) && __STDC_WANT_SECURE_LIB__
    sprintf_s((char *)q->description, sizeof("Intel family  model ") + 10, "Intel family %u model %u",
#else
	snprintf((char *)q->description, sizeof("Intel family  model ") + 10, "Intel family %u model %u",
#endif
...
#if defined(_MSC_VER) && __STDC_WANT_SECURE_LIB__
    sprintf_s((char *)q->description, sizeof("PowerPC cpusubtype ") + 10, "PowerPC cpusubtype %u", cpusubtype);
#else
	snprintf((char *)q->description, sizeof("PowerPC cpusubtype ") + 10, "PowerPC cpusubtype %u", cpusubtype);
#endif
...
uint32_t nfat_archs)
{
	...
	case CPU_TYPE_I386:
		...
		if((cpu_subtype_t)(CPU_SUBTYPE_INTEL_FAMILY(fat_archs[i].cpusubtype &
			...
			~CPU_SUBTYPE_MASK)) < (cpu_subtype_t)lowest_model){
	...
	case CPU_TYPE_POWERPC:
		...
		for(i = 0; i < nfat_archs; i++){
		    if((cpu_type_t)(fat_archs[i].cputype & ~CPU_SUBTYPE_MASK) != (cpu_type_t)cputype)
			continue;
		    if((fat_archs[i].cpusubtype & ~CPU_SUBTYPE_MASK) ==
