...
enum NXByteOrder target_byte_sex)
{
	...
	for(i = 0; i < 16; i++)		/* 16 doubles */
        ...
        (double)OSSwapInt64((int64_t)fpu->fpu.fpu_fr.Fpu_dregs[i]);

	fpu->fpu.Fpu_q[0].FQu.whole = (double)OSSwapInt64((int64_t)fpu->fpu.Fpu_q[0].FQu.whole);
	fpu->fpu.Fpu_q[1].FQu.whole = (double)OSSwapInt64((int64_t)fpu->fpu.Fpu_q[1].FQu.whole);
