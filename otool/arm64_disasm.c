 ...
 * from disassemble_info.  If symbolic information set into op_info then this function
...
uint32_t r_type,
struct _disassemble_info *_disassemble_info,
struct LLVMOpInfo1 *op_info)
{
    uint32_t n_strx;

	if(r_symbolnum >= _disassemble_info->nsymbols)
	    return(0);
	n_strx = _disassemble_info->symbols[r_symbolnum].n_un.n_strx;
	if(n_strx <= 0 || n_strx >= _disassemble_info->strings_size)
	    return(0);
	op_info->AddSymbol.Present = 1;
	op_info->AddSymbol.Name = _disassemble_info->strings + n_strx;
	switch(r_type){
 ...
 * information is a pointer to the struct _disassemble_info that was passed when
...
void *TagBuf)
{
	...
	_disassemble_info = (struct _disassemble_info *)DisInfo;
	...
	if(Offset != 0 || Size != 4 || TagType != 1 ||
	   _disassemble_info->verbose == FALSE)
	    return(0);

	/* First look in the section relocations if any. */
	sect_offset = (uint32_t)(Pc - _disassemble_info->sect_addr);
	for(i = 0; i < _disassemble_info->nrelocs; i++){
	    if(_disassemble_info->relocs[i].r_address == (int32_t)sect_offset){
		if(_disassemble_info->relocs[i].r_type == ARM64_RELOC_ADDEND){
		    if(i+1 < _disassemble_info->nrelocs &&
		       _disassemble_info->relocs[i+1].r_address == (int32_t)sect_offset){
			if(value == 0){
			    value = _disassemble_info->relocs[i].r_symbolnum;
			    op_info->Value = value;
			}
			i++;
		    }
		...
		if(_disassemble_info->relocs[i].r_extern)
		    return(set_OpInfo_from_reloc(
				_disassemble_info->relocs[i].r_symbolnum,
				_disassemble_info->relocs[i].r_type, _disassemble_info, op_info));
	    }
	}

	/* Second look in the image's external relocations if any. */
	for(i = 0; i < _disassemble_info->next_relocs; i++){
	    if(_disassemble_info->ext_relocs[i].r_address == Pc){
		if(_disassemble_info->ext_relocs[i].r_type == ARM64_RELOC_ADDEND){
		    if(i+1 < _disassemble_info->next_relocs &&
		       _disassemble_info->ext_relocs[i+1].r_address == Pc){
			if(value == 0){
			    value = _disassemble_info->ext_relocs[i].r_symbolnum;
			    op_info->Value = value;
			}
			i++;
		    }
		...
		return(set_OpInfo_from_reloc(
			    _disassemble_info->ext_relocs[i].r_symbolnum,
			    _disassemble_info->ext_relocs[i].r_type, _disassemble_info,
			    op_info));
	    }
	}
...
BOOL *cfstring)
#else
unsigned char *classref,
unsigned char *selref,
unsigned char *msgref,
unsigned char *cfstring)
#endif
{
...
#else
    unsigned char classref, selref, msgref, cfstring;
#endif
	...
	if(*ReferenceName != NULL){
		...
		*ReferenceType =
		...
		if(info->selector_name != NULL){
			...
			strncpy(info->method, "+[", 5 + strlen(info->class_name) +
                    strlen(info->selector_name));
			strncat(info->method, info->class_name, 5 + strlen(info->class_name) +
                    strlen(info->selector_name) - strlen(info->method));
			strncat(info->method, " ", 5 + strlen(info->class_name) +
                    strlen(info->selector_name) - strlen(info->method));
			strncat(info->method, info->selector_name, 5 + strlen(info->class_name) +
                    strlen(info->selector_name) - strlen(info->method));
			strncat(info->method, "]", 5 + strlen(info->class_name) +
                    strlen(info->selector_name) - strlen(info->method));
			*ReferenceName = info->method;
			...
			strncpy(info->method, "-[x0 ", 7 + strlen(info->selector_name));
			strncat(info->method, info->selector_name, 7 + strlen(info->selector_name) - strlen(info->method));
			strncat(info->method, "]", 7 + strlen(info->selector_name) - strlen(info->method));
			*ReferenceName = info->method;
		    }
		}
	    }
		...
		*ReferenceType =
		...
		if(info->selector_name != NULL){
		    ...
		    strncpy(info->method, "-[[x0 super] ", 15 + strlen(info->selector_name));
		    strncat(info->method, info->selector_name, 15 + strlen(info->selector_name) - strlen(info->method));
		    strncat(info->method, "]", 15 + strlen(info->selector_name) - strlen(info->method));
		    *ReferenceName = info->method;
		    info->class_name = NULL;
		}
	    }
...
const char **ReferenceName)
{
	...
	info = (struct _disassemble_info *)DisInfo;
	 ...
	 */
	    ...
	    addxri_inst = (uint32_t)SymbolValue;
	 ...
	 */
	    ...
	    ldrxui_inst = (uint32_t)SymbolValue;
...
cpu_subtype_t cpusubtype)
{
	...
	llvm_create_disasm
#endif
	    ("arm64-apple-darwin10", mcpu_default, &disassemble_info, 1, GetOpInfo,
	     SymbolLookUp);
	return(dc);
}
...
LLVMDisasmContextRef dc)
{
	...
	disassemble_info.verbose = verbose;
	disassemble_info.relocs = relocs;
	disassemble_info.nrelocs = nrelocs;
	disassemble_info.ext_relocs = ext_relocs;
	disassemble_info.next_relocs = next_relocs;
	disassemble_info.loc_relocs = loc_relocs;
	disassemble_info.nloc_relocs = nloc_relocs;
	disassemble_info.dbi = dbi;
	disassemble_info.ndbi = ndbi;
	disassemble_info.symbols = symbols;
	disassemble_info.nsymbols = nsymbols;
	disassemble_info.sorted_symbols = sorted_symbols;
	disassemble_info.nsorted_symbols = nsorted_symbols;
	disassemble_info.strings = strings;
	disassemble_info.strings_size = strings_size;
	disassemble_info.load_commands = load_commands;
	disassemble_info.object_byte_sex = object_byte_sex;
	disassemble_info.indirect_symbols = indirect_symbols;
	disassemble_info.nindirect_symbols = nindirect_symbols;
	disassemble_info.ncmds = ncmds;
	disassemble_info.sizeofcmds = sizeofcmds;
	disassemble_info.object_addr = object_addr;
	disassemble_info.object_size = object_size;
	disassemble_info.sect = sect;
	disassemble_info.left = left;
	disassemble_info.addr = (uint32_t)addr;
	disassemble_info.sect_addr = sect_addr;
	disassemble_info.method = NULL;
	disassemble_info.demangled_name = NULL;
