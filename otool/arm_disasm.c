...
#else
typedef unsigned char bfd_boolean;
#endif
...
#if defined(_WIN32) || defined(WIN32)
  BOOL(*print_immediate_func)
      (bfd_vma pc, unsigned int imm, struct disassemble_info *info,
       BOOL pool);
#else
  unsigned char (*print_immediate_func)
    (bfd_vma pc, unsigned int imm, struct disassemble_info *info,
     unsigned char pool);
#endif
...
const char **ReferenceName)
{
	...
	if(*ReferenceType == LLVMDisassembler_ReferenceType_In_Branch){
	    ...
	    *ReferenceName = guess_literal_pointer((const uint32_t)SymbolValue, (const uint32_t)ReferencePC,
						   ReferenceType, info);
	    if(*ReferenceName == NULL)
		*ReferenceType = LLVMDisassembler_ReferenceType_InOut_None;
	}
...
struct opcode32
{
  uint32_t arch;		/* Architecture defining this insn.  */
  uint32_t value, mask;		/* Recognise insn if (op&mask)==value.  */
...
{
  /* APPLE LOCAL */
  {ARM_EXT_V1, 0xe7ffdefe, 0xffffffff, "trap"},
	  ...
	  for (c = insn->assembler; *c; c++)
	      ...
	      if (*c == '%')
		{
		  switch (*++c)
			...
			c = arm_decode_bitfield (c, given, &value3, &width);

			switch (*c)
			  {
			  case 'r':
			    func (stream, "%s", arm_regnames[value3]);
			    break;
			  case 'D':
			    func (stream, "d%ld", value3);
			    break;
			  case 'Q':
			    if (value & 1)
			      func (stream, "<illegal reg q%ld.5>", value3 >> 1);
			    else
			      func (stream, "q%ld", value3 >> 1);
			    break;
			  case 'd':
			    func (stream, "%ld", value3);
			    break;
                          case 'k':
                            {
                              int from = (given & (1 << 7)) ? 32 : 16;
                              func (stream, "%ld", from - value3);
                            }
                            break;
                            
			  case 'f':
			    if (value > 7)
			      func (stream, "#%s", arm_fp_const[value3 & 7]);
			    else
			      func (stream, "f%ld", value3);
			    break;

			  case 'w':
			    if (width == 2)
			      func (stream, "%s", iwmmxt_wwnames[value3]);
			    else
			      func (stream, "%s", iwmmxt_wwssnames[value3]);
			    break;

			  case 'g':
			    func (stream, "%s", iwmmxt_regnames[value3]);
			    break;
			  case 'G':
			    func (stream, "%s", iwmmxt_cregnames[value3]);
			    break;

			  case 'x':
			    func (stream, "0x%lx", value3);
			    break;
			  ...
			  case '\'':
			    ...
			    break;
			  case '?':
			    func (stream, "%c", c[(1 << width) - (int)value3]);
			    c += 1 << width;
			    break;
			  default:
			    abort ();
	  ...
	  for (c = insn->assembler; *c; c++)
	      ...
	      if (*c == '%')
		{
		  switch (*++c)
			...
			if (align)
			  {
                            int align2 = (8 * (type + 1)) << size;
                            if (type == 3)
                              align2 = (size > 1) ? align2 >> 1 : align2;
			    if (type == 2 || ((type == 0) && !size))
			      func (stream, ", :<bad align %d>", align2);
			    else
			      func (stream, ", :%d", align2);
			  }
			func (stream, "]");
			if (rm == 0xd)
			  func (stream, "!");
	      ...
	      if (*c == '%')
		{
		  switch (*++c)
		    ...
		    case 'B':
		      ...
		      {
			bfd_vma address;
			bfd_vma offset = (bfd_vma)0;

			if (given & 0x00800000)
			  /* Is signed, hi bits should be ones.  */
			  offset = (bfd_vma)((-1) ^ 0x00ffffff);
			   ...
			   language instruction encodes LSB and MSB.  */
			{
			  ...
			  int32_t width2 = msb - lsb + 1;
			  if (width2 > 0)
			    func (stream, "#%lu, #%lu", lsb, width2);
			  else
...
struct disassemble_info *info)
{
    ...
    bfd_vma r_address = (bfd_vma)(pc - info->sect_addr);

    if(info->verbose){
	/* If there's a relocation at this address, include the referenced
	   symbol.  */
	for(i = 0; i < (int32_t)nrelocs; i++){
	    if((relocs[i].r_address == (int32_t)r_address) && relocs[i].r_extern){
...
#endif
{
	...
	if(pool){
		 ...
		 */
		offset = -(int32_t)(value + pair_r_value - r_value);
		if(add != NULL)
...
struct disassemble_info *info)
{
    ...
    UNREFERENCED_PARAMETER(memaddr);
    UNREFERENCED_PARAMETER(info);
...
uint32_t ninsts)
{
    UNREFERENCED_PARAMETER(cpusubtype);
    ...
    int32_t high, low, mid;
		...
		do{
			...
			*in_thumb2 = TRUE;
		    ...
		    mid++;
		    if(mid > (int32_t)nsorted_symbols ||
