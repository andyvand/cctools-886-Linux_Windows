...
#include "../include/stuff/bool.h"
#include "../include/stuff/arch.h"

typedef enum ofile_type
{
    OFILE_UNKNOWN,
    OFILE_FAT,
    OFILE_ARCHIVE,
...
typedef struct ofile {
    ...
    unsigned char	   toc_bad;	    /* the toc needs to be rebuilt */
	...
	*dylib_module64;	    /*  this module. */
    ...
    unsigned char headers_swapped;	    /* true if the headers have already been
...
#else
__private_extern__ unsigned char ofile_map(
#endif
...
#else
__private_extern__ unsigned char ofile_map_from_memory(
#endif
    char *addr,
    uint64_t size,
    const char *file_name,
...
__private_extern__ void ofile_unmap(
    struct ofile *ofile);
__private_extern__ unsigned char ofile_first_arch(
    struct ofile *ofile);
__private_extern__ unsigned char ofile_next_arch(
    struct ofile *ofile);
__private_extern__ unsigned char ofile_first_member(
    struct ofile *ofile);
__private_extern__ unsigned char ofile_next_member(
    struct ofile *ofile);
__private_extern__ unsigned char ofile_specific_member(
    const char *object_name,
    struct ofile *ofile);
__private_extern__ unsigned char ofile_first_module(
    struct ofile *ofile);
__private_extern__ unsigned char ofile_next_module(
    struct ofile *ofile);
__private_extern__ unsigned char ofile_specific_module(
