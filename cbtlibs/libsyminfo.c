...
struct cmd_flags {
    unsigned char g;        /* select only global symbols */
    unsigned char u;        /* select only undefined symbols */
    unsigned char d;        /* select only defined symbols, opposite of u */
    unsigned char getLibDeps; /* Get the library dependency info like
                             SUB_UMBRELLA, SUB_FRAMEWORK, etc */
    unsigned char import;   /* Flag to identify were looking for imported symbols
                           data */
};
...
char *fileName)
{
    ...
    kern_return_t r;
...
#ifndef bzero
        memset(rList, 0, sizeof(_SymInfoList));
#else
...
#ifndef bzero
        memset(gInfo, 0, sizeof(struct selectedSymbolListInfo));
#else
        ...
        if((r = vm_deallocate(mach_task_self(), (vm_address_t)gInfo->mappedFile,
            ...
            fprintf(stderr, "Can't vm_deallocate mapped memory for file: "
                   "%s",fileName);
#elif defined(__linux__)
            err(r, "Can't vm_deallocate mapped memory for file: "
                   "%s",fileName);
#else
...
#ifndef bzero
        memset(deps, 0, sizeof(_SymInfoDependencies));
#else
...
void *cookie)
{
    UNREFERENCED_PARAMETER(arch_name);
    ...
    uint32_t ncmds, mh_flags;
        ...
        for(i = self->nExports - nsymbols; i < self->nExports; i++){
            ...
            if(archInfo == NULL){
                ...
                sprintf_s(archString, sizeof(archString), "%d", ofile->mh_cputype);
#else
                snprintf(archString, sizeof(archString), "%d", ofile->mh_cputype);
#endif
...
#ifndef bzero
            memset(self->dependencies, 0, sizeof(_SymInfoDependencies));
#else
...
#if defined(_MSC_VER)
                    strcat_s(returnLongName, strlen(longName) + 1, longName);
#else
                    strncat(returnLongName, longName, strlen(longName) + 1);
#endif
        ...
        if(ofile->dylib_module != NULL){
            ...
            for(i = 0; i < nrefsym; i++){
                ...
                if(flags == REFERENCE_FLAG_UNDEFINED_NON_LAZY ||
                   ...
                   flags == (uint16_t)REFERENCE_FLAG_PRIVATE_UNDEFINED_LAZY){
                    if(ofile->mh != NULL)
                    ...
                    if(flags == REFERENCE_FLAG_UNDEFINED_NON_LAZY ||
                       flags == REFERENCE_FLAG_UNDEFINED_LAZY)
                        ...
                        symbol.nl.n_type = (uint16_t)N_UNDF;
                    symbol.nl.n_desc = (uint16_t)((symbol.nl.n_desc &~ REFERENCE_TYPE) | flags);
                    symbol.nl.n_value = 0;
