...
char *out_file_name)
{
	...
	output_addr = (char *)malloc(output_size);
	if (output_addr == (vm_address_t)0)
	    as_fatal("can't malloc() buffer for output file of size %u",
		     output_size);
#else
	 ...
	 * that exist.
	 */
	if(_stat(out_file_name, &stat_buf) != -1){
	    if(stat_buf.st_mode & S_IFREG)
		(void)unlink(out_file_name);
	}
	if((fd = _open(out_file_name, O_WRONLY | O_CREAT | O_TRUNC, 0666)) == -1)
	    as_fatal("can't create output file: %s", out_file_name);
	if(_write(fd, output_addr, output_size) != (int)output_size)
