...
char **envp*/)
{
...
#if defined(_WIN32) || defined(WIN32)
    char *prefix, buf[MAX_PATH], resolved_name[MAX_PATH];
#else
    ...
    unsigned char oflag_specified, qflag, Qflag, some_input_files;
	...
	prefix = realpath(p, resolved_name);
        if (prefix == NULL)
        {
            err(1, "realpath(3) for %s failed", p);
            exit(-1);
        }
        p = rindex(prefix, '/');
	if(p != NULL)
    ...
    strcpy_s(p + strlen(p), bufsize - strlen(p), "/");
    strcpy_s(p + strlen(p), bufsize - strlen(p), str);
#else
    strncpy(cmdpref + strlen(p), "/" , bufsize - strlen(p));
    strncpy(cmdpref + strlen(p), str, bufsize - strlen(p));
#endif
#endif
...
#if defined(__LP64__) || defined(__x86_64__) || defined(_AMD64_) || defined(_M_AMD64) || defined(_X64_) || defined(_M_X64)
		/*
