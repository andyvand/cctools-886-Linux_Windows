 ...
 */
static unsigned char default_dyld_executable = FALSE;
#endif /* NMEDIT */
...
#else
static unsigned char *nmedits = NULL;
#endif
...
#ifndef NMEDIT
static unsigned char strip_symtab(
    struct arch *arch,
    struct member *member,
    struct object *object,
...
static unsigned char private_extern_reference_by_module(
    uint32_t symbol_index,
    struct dylib_reference *refs,
    uint32_t nextrefsyms);

static unsigned char symbol_pointer_used(
    uint32_t symbol_index,
...
#ifdef NMEDIT
static unsigned char edit_symtab(
    struct arch *arch,
    struct member *member,
    struct object *object,
    struct nlist *symbols,
...
unsigned char all_archs)
{
    ...
    unsigned char unix_standard_mode;
    int cwd_fd;
    char *rename_file;
#ifndef NMEDIT
    char *p;
...
unsigned char all_archs)
{
    ...
    unsigned char arch_process, any_processing, *arch_flag_processed, family;
	...
	if(narch_flags != 0)
	    arch_flag_processed = allocate(narch_flags * sizeof(unsigned char));
	memset(arch_flag_processed, '\0', narch_flags * sizeof(unsigned char));
	for(i = 0; i < narchs; i++){
	    /*
...
uint32_t nextrefsyms)
{
    ...
    unsigned char has_dwarf, hack_5614542;
...
uint32_t nextrefsyms)
{
    ...
    unsigned char global_symbol_found;
    char *global_name, save_char;
    unsigned char dwarf_debug_map;
    enum byte_sex host_byte_sex;
    int32_t missing_reloc_symbols;
    unsigned char edit_symtab_return;
    ...
    uint64_t n_value;
    unsigned char warned_about_global_coalesced_symbols;
	...
	if(nmedits != NULL)
	    free(nmedits);
	nmedits = allocate(nsyms * sizeof(unsigned char));
	for(i = 0; i < nsyms; i++)
	    nmedits[i] = FALSE;
