 ...
 * equality.
 */
static unsigned char zero_ar_date = FALSE;
...
struct cmd_flags {
    ...
    unsigned char ranlib;	/* set if this is run as ranlib not libtool */
    unsigned char s;	/* sort the table of contents */
    unsigned char a;	/* don't sort the table of contents (original form) */
    unsigned char c;	/* include commmon symbols in the table of contents */
    unsigned char t;	/* just "touch" the archives to get the date right */
    unsigned char f;	/* warn if the output archive is fat,used by ar(1) -s */
    unsigned char q;	/* only write archive if NOT fat, used by ar(1) */
    char *output;	/* the output file specified by -o */
    unsigned char final_output_specified; /* if -final_output is specified */
    unsigned char dynamic;	/* create a dynamic shared library, static by default */
    ...
    unsigned char verbose;	/* print exec(2) commands run */
    struct arch_flag
	arch_only_flag;	/* the -arch_only flag if specified */
    unsigned char		/* set if either -prebind or -noprebind is seen */
	prebinding_flag_specified;
    unsigned char		/* set if -prebind is seen or the LD_PREBIND */
	prebinding;	/*  environment variable is set (and -noprebind isn't)*/
    unsigned char		/* set if either -all_load or -noall_load is seen */
	all_load_flag_specified;
    unsigned char		/* set if -all_load is seen (and -noall_load isn't) */
	all_load;
    unsigned char		/* set with -L (the default) off with -T, for -static */
	use_long_names; /* use 4.4bsd extended format 1 for long names */
    unsigned char L_or_T_specified;
    unsigned char		/* set if the environ var LD_TRACE_ARCHIVES is set */
	ld_trace_archives;
	const char *	/* LD_TRACE_FILE if set and LD_TRACE_ARCHIVES is set, or NULL */
	trace_file_path;
    unsigned char		/* set if -search_paths_first is specified */
	search_paths_first;
    unsigned char noflush;	/* don't use the output_flush routine to flush the
			   static library output file by pages */
    uint32_t debug;	/* debug value to debug output_flush() routine */
    unsigned char		/* don't warn if members have no symbols */
...
struct arch {
    ...
    unsigned char toc_long_name;    /* use the long name in the output */
...
struct member {
	...
	*load_commands;
...
#ifdef LTO_SUPPORT
    unsigned char lto_contents;	    /* TRUE if this member has lto contents */
    ...
    unsigned char output_long_name;	    /* use the extended format #1 for the
...
static int toc_index1_qsort(
    const struct toc *toc1,
    const struct toc *toc2);
static unsigned char toc_symbol(
    struct nlist *symbol,
    struct section **sections);
static unsigned char toc_symbol_64(
...
static unsigned char toc(
    ...
    uint8_t n_type,
    uint64_t n_value,
    unsigned char attr_no_toc);
static unsigned char check_sort_tocs(
    struct arch *arch,
    char *output,
    unsigned char library_warnings);
...
char **envp)
{
    ...
    uint32_t temp;
    int oumask, numask;
    unsigned char lflags_seen, bad_flag_seen, Vflag;
	...
	for(i = 1; i < argc; i++){
		...
		if(strcmp(argv[i], "-o") == 0){
			...
			*dirname = '\0';
		    ...
		    {
			fatal("can't open file list file: %s", filelist);
			exit(-1);
		    }
#else
...
#if defined(__linux__)
		    {
			fatal("can't stat file list file: %s", filelist);
			exit(-1);
		    }
#else
...
void)
{
    ...
    struct ofile *ofiles;
    char *file_name;
    unsigned char flag, ld_trace_archive_printed;
...
struct ofile *ofile)
{
    ...
    unsigned char some_tocs, same_toc, different_offsets;
	...
	if(cmd_flags.ranlib == TRUE && narchs == 1 &&
	   ...
	   archs[0].toc_strsize == ofile->toc_strsize){
	    ...
	    library = malloc(library_size);

	    if (library == NULL)
		mach_fatal(r, "can't malloc() buffer for output file: %s "
	    ...
	    if(close(fd) == -1){
#if defined(__linux__)
		fatal("can't close output file: %s", output);
		exit(-1);
#else
...
fail_to_update_toc_in_place:
	...
	if (library == NULL)
	    mach_fatal(r, "can't malloc() buffer for output file: %s of "
...
update_toc_ar_dates:
...
#if defined(__linux__)
	    fatal("can't stat file output file: %s", output);
	    exit(-1);
#else
	...
	{
#if defined(__linux__)
	    fatal("can't set the modifiy times in output file: %s",
			 output);
	    exit(-1);
#else
...
{ 
	...
	if((cmd_flags.debug & (1 << 1)) || (cmd_flags.debug & (1 << 0)))
	    ...
	    if(write(fd, library + write_offset, write_size) !=
	       (int)write_size)
#if defined(__linux__)
		fatal("can't write to output file");
		exit(-1);
#else
...
char *output)
{
    ...
    char *p, *filelist;
    struct stat stat_buf;
    unsigned char use_force_cpusubtype_ALL;
    const struct arch_flag *family_arch_flag;
	...
	for(i = 0; i < narchs || (i == 0 && narchs == 0); i++){
		...
		add_execute_list(cmd_flags.Ldirs[j]);
            ...
            // so.
            unsigned char use_dashl_dylib1o = TRUE;
            if (sdkroot && sdkroot[0] == '/') {
              // Construct the path to the object file.
              char *sdk_dylib1o_path;

#if defined(_WIN32) || defined(WIN32) || defined(__linux__)
	      size_t sz = strlen("/usr/lib/dylib1.o") + strlen(sdkroot) + 1; 
	      sdk_dylib1o_path = malloc(sz);
	      int res = snprintf(sdk_dylib1o_path, sz, "%s/usr/lib/dylib1.o",
                                 sdkroot);
#else
...
char *output)
{
    ...
    char *strings;
    unsigned char sorted, is_toc_symbol;
    char *ar_name;
