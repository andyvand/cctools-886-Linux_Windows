...
struct layout_info {
    ...
    unsigned char split;
    unsigned char use_debug_region;
...
struct info {
    ...
    unsigned char allocate_flat_increasing;
...
static char * get_image_file_name(
    struct info *info,
    char *install_name,
    unsigned char try_symroot,
    unsigned char no_error_if_missing);
...
char **envp)
{
    ...
    struct seg_addr_table *entry;
    struct layout_info *layout_info;
    unsigned char seg1addr_specified, segs_read_only_addr_specified,
	      segs_read_write_addr_specified, relayout, update, create,
	      checkonly, update_overlaps, allocate_flat_specified,
	      relayout_nonsplit;
    unsigned char found, is_framework, next_flat, next_split, next_debug;
    unsigned char operation_specified, from_dylib_table, create_dylib_table;
