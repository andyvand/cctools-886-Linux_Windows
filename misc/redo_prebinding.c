...
#define malloc_create_zone(a,b) NULL
#define malloc_zone_realloc(a,b,c) realloc(b,c)
#define malloc_destroy_zone(a)
#define malloc_set_zone_name(a,b)
#define malloc_zone_malloc(a,b) malloc(b)
#endif
...
static unsigned char check_only = FALSE;
static unsigned char seen_a_non_64_bit = FALSE;

/* -i option, ignore non-prebound files */
static unsigned char ignore_non_prebound = FALSE;

/* -z option, zero out prebind checksum */
static unsigned char zero_out_prebind_checksum = FALSE;

/* -p option, check for non-prebound files */
static unsigned char check_for_non_prebound = FALSE;

/* -d option, check for dynamic library files */
static unsigned char check_for_dylibs = FALSE;
static unsigned char seen_a_dylib = FALSE;
static unsigned char seen_a_non_dylib = FALSE;
...
static unsigned char arch_swapped = FALSE;
...
static unsigned char arch_split_segs = FALSE;
...
static unsigned char arch_force_flat_namespace = FALSE;
...
struct lib {
    ...
    unsigned char LC_PREBOUND_DYLIB_found;
     ...
     */
    unsigned char sub_images_setup;
    uint32_t *sub_images;
    uint32_t nsub_images;

    unsigned char two_level_debug_printed;
};
...
static unsigned char has_resource_fork(
    char *filename);

static void process_archs(
    struct arch *archs,
    uint32_t narchs,
    unsigned char has_resource_fork);
...
static unsigned char load_archs_libraries(void);

static unsigned char load_library(
    char *file_name,
    struct dylib_command *dl_load,
    unsigned char time_stamps_must_match,
    uint32_t *image_pointer);

static unsigned char load_dependent_libraries(void);

static void print_two_level_info(
    struct lib *lib);

static unsigned char setup_sub_images(
    struct lib *lib,
...
static void setup_symbolic_info(unsigned char missing_arch);
...
static unsigned char check_dylibs_for_reference(
    char *symbol_name);
...
static unsigned char lookup_symbol_in_arch(
    char *name,
    struct nlist **symbol,
    enum link_state **module_state,
    struct lib **lib,
...
static unsigned char lookup_symbol_in_lib(
    char *name,
    struct lib *primary_lib,
    struct nlist **symbol,
    enum link_state **module_state,
...
static unsigned char check_pb_la_ptr_reloc_cputype(
...
char *envp[])
{
    ...
    unsigned char verbose, calculate_input_prebind_cksum, write_to_stdout;
    unsigned short mode;
    uid_t uid;
    gid_t gid;
 ...
 * parameter.
 */
static unsigned char check_if_needed = FALSE;
static unsigned char redo_prebinding_needed = FALSE;
...
uint32_t *throttle)
{
    ...
    uid_t uid;
    gid_t gid;
    unsigned char calculate_input_prebind_cksum;
...
uint32_t *outlen)
{
    ...
    uid_t uid;
    gid_t gid;
    unsigned char calculate_input_prebind_cksum, seen_archive;
...
void)
{
    ...
    struct dylib_command *dl_load;
    uint32_t *dependent_images, *image_pointer;
    unsigned char some_images_setup;
    uint32_t ncmds;
...
uint32_t *image_pointer)
{
    ...
    struct load_command *lc;
    struct dylib_command *dl_id;
    unsigned char already_loaded, is_framework;
    char *suffix;
    struct stat stat_buf;
    uint32_t ncmds;
...
char *name,
struct lib *primary_lib,
unsigned char weak, /* the symbol is allowed to be missing, weak */
struct nlist **symbol,
enum link_state **module_state,
struct lib **lib,
uint32_t *isub_image,
...
uint32_t vmslide)
{
    ...
    unsigned char found, prebind_all_twolevel_modules;
